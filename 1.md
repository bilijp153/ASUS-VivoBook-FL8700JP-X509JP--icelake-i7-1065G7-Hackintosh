...
|------------------|------------------|----------------------------|----------------------------|
| [🖥️ 系统支持](#️-系统支持)| [⚙️ 安装指南](#️-安装指南) | [❓ 常见问题](#-常见问题) |
...
6. 多次重启后进入Mac开箱界面 (注意：开箱界面里提示登录AppleID时必须先**跳过**后续在**系统设置**中登录)
...
2. 多次点按生成然后点击**SystemSerialNumber**后面的检查去Apple网站检查序列号**可用性**(⚠️注意：必须生成**唯一三码**且验证验证序列号状态显示 **"无效序列号"**！)
3. 成功生成后保存**config**文件并重启电脑(⚠️注意：生成后就可以登录你的AppleID了！)
